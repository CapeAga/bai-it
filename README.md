...
不认识的词用虚线标出来。插件会先尽量用更简单的英文解释这个词，中文留作兜底，不会默认把你直接带回翻译模式。内置 31,000+ 词条的离线词典，不需要联网。你觉得已经认识的词，点一下标记掌握，以后就不再打扰你。

### 手动掰句

遇到自动没拆的句子，觉得看不懂？点一下旁边的按钮，手动"掰"一下。插件会优先给你一版更容易懂的英文，再把中文留作备用，帮助你慢慢脱离翻译依赖。
...
2. 安装依赖并构建：
   ...
   npm run build
   ```
   项目默认不会下载 Puppeteer 自带浏览器，而是复用你本机已安装的 Chrome，所以安装不应再卡在浏览器下载步骤。
3. 打开 Chrome，进入 `chrome://extensions/`
4. 打开右上角「开发者模式」
...
3. **手动掰句** — 对看不懂的句子点触发按钮，先看更简单的英文，不够再展开中文
